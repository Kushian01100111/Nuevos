...
function presses(phrase) {
    ...
    for (const char of phrase) {
        let chart = /\d/.test(char) ? Number(char) : char.toUpperCase();
        let arrIndex = candyBar.findIndex(arr => arr.includes(chart));
        
        if (arrIndex !== -1) {
            let charIndex = candyBar[arrIndex].indexOf(chart);
            countButtons += charIndex + 1;
        }
    }
...
console.log(presses("WHERE DO U WANT 2 MEET L8R"), 47, "should work for phrases with numbers")