...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth){
    ...
    let currentStartPrice = startPriceOld,
        currentPriceNew = startPriceNew,
        currentPercent = percentLossByMonth, 
        savingsByMonth = 0,
        valueEndMonth = 0,
        result= [];
  
  for(let i=1; valueEndMonth<=currentPriceNew; i++){
      if(i%2=== 0){
        currentPercent+=0.5
      }
    currentStartPrice=currentStartPrice-(currentStartPrice*(currentPercent/100));
    currentPriceNew=currentPriceNew-(currentPriceNew*(currentPercent/100));
    savingsByMonth+=savingperMonth;
    valueEndMonth= currentStartPrice + savingsByMonth;
...
console.log(nbMonths(18000, 32000, 1500, 1.25), [8, 597]);